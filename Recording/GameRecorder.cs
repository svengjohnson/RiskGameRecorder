...
public sealed class GameRecorder
{
    ...
    public void ProcessSnapshot(FullMemorySnapshot snap, GameMemoryReader reader)
    {
        ...
        {
            case State.Idle:
                if (snap.State == GameState.InGame && IsValidGameId(snap.GameId))
                {
            ...
            case State.Recording:
                if (snap.State == GameState.GameOver)   { FinalizeAndSave(); break; }
    ...
    void StartRecording(FullMemorySnapshot snap, GameMemoryReader reader)
    {
        ...
        // Blizzard territories at game start
    ...
    void ProcessTick(FullMemorySnapshot snap, GameMemoryReader reader)
    {
        ...
        // Check for alliance changes — record full snapshot against current player's turn
        if (!string.IsNullOrEmpty(_currentTurnId) && round.PlayerTurns.ContainsKey(_currentTurnId))
        {
    ...
    {
        if (_game != null && _game.RoundInfo.Count > 0)
        {
            _game.GameInfo.GameDuration = SnapTime();

            // Append game_over snapshot to the current player's turn if one is active
            if (!string.IsNullOrEmpty(_currentTurnId) && _game.RoundInfo.ContainsKey(_currentRoundKey))
            {
                var lastRound = _game.RoundInfo[_currentRoundKey];
    ...
    void SaveToDisk()
    {
        ...
        int version   = _game!.Metadata.Version;
        ...
        var json = JsonSerializer.Serialize(_game, new JsonSerializerOptions { WriteIndented = true });
